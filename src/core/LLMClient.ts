...
import {
  buildAzureChatPayload,
  extractAzureErrorMessage,
  resolveModelCapabilities,
} from './llmCapabilities';
...
export class LLMClient {
  ...
  private loadLLMConfig(): any {
    try {
      ...
      const llmConfigPath = path.join(process.cwd(), 'resources', 'config', 'llm.json');
  ...
  constructor(config?: {
    ...
    this.model = this.resolveConfigValue(activeProviderConfig.model) || 'gemini-2.5-flash';
  ...
  private resolveConfigValue(value: unknown): string | undefined {
    if (typeof value !== 'string') return undefined;
    const trimmed = value.trim();
    if (!trimmed || trimmed.includes('YOUR_')) return undefined;
  ...
  private resolveKeyForProvider(provider: string): string {
    ...
    const configuredApiKey = this.resolveConfigValue(provConfig.apiKey);
    ...
    if (configuredApiKey) {
      ...
      if (provider === 'azure') {
        const endpoint = this.resolveConfigValue(provConfig.endpoint);
        const apiVersion = this.resolveConfigValue(provConfig.apiVersion);
        const deploymentId = this.resolveConfigValue(provConfig.deploymentId);
        if (endpoint) {
        ...
        if (deploymentId) {
          process.env.AZURE_OPENAI_DEPLOYMENT = deploymentId;
        }
      } else if (provider === 'aws') {
        process.env.AWS_ACCESS_KEY_ID = configuredApiKey;
        const secretKey = this.resolveConfigValue(provConfig.secretKey);
        const region = this.resolveConfigValue(provConfig.region);
        if (secretKey) process.env.AWS_SECRET_ACCESS_KEY = secretKey;
        if (region) process.env.AWS_BEDROCK_REGION = region;
      } else if (provider === 'gcp') {
        const projectId = this.resolveConfigValue(provConfig.projectId);
        const region = this.resolveConfigValue(provConfig.region);
        if (projectId) process.env.GCP_PROJECT_ID = projectId;
        if (region) process.env.GCP_LOCATION = region;
      } else if (provider === 'ollama') {
        const endpoint = this.resolveConfigValue(provConfig.endpoint);
        if (endpoint) process.env.OLLAMA_ENDPOINT = endpoint;
      }
      return configuredApiKey;
    }

    // Secondary Check for AWS Secret Key if apiKey was set from env
    const configuredSecretKey = this.resolveConfigValue(provConfig.secretKey);
    if (provider === 'aws' && configuredSecretKey) {
      process.env.AWS_SECRET_ACCESS_KEY = configuredSecretKey;
    }
    ...
    for (const modelAttempt of modelsToTry) {
      ...
      for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
        try {
          ...
          lastError = error;
          const azureDetail = extractAzureErrorMessage(error);
          const detail = azureDetail && azureDetail !== error.message
            ? `${error.message} — ${azureDetail}`
            : error.message;
          console.warn(`[LLM Warning] Attempt ${attempt}/${this.retryAttempts} failed for model ${currentModel}: ${detail}`);
  ...
  private async callOpenAI(model: string, messages: LLMMessage[]): Promise<LLMResponse> {
    ...
    const payload: Record<string, unknown> = {
      model,
      messages,
      [caps.tokenLimitField]: this.maxTokens,
    };
  ...
  private async callAzureOpenAI(model: string, messages: LLMMessage[]): Promise<LLMResponse> {
    ...
    const payload = buildAzureChatPayload(deployment, {
      messages,
