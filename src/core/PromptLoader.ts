...
const PROMPTS_ROOT = path.join(process.cwd(), 'resources', 'prompts');

/**
 * Loads editable Markdown prompts from `resources/prompts/`.
 */
export class PromptLoader {
  ...
  public static load(relativePath: string): string {
    ...
    if (!fs.existsSync(full)) {
      throw new Error(`Prompt file not found: resources/prompts/${relativePath}`);
    }
