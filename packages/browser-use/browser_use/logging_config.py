...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	...
	class WebPilotLogFormatter(logging.Formatter):
		def __init__(self, fmt, log_level):
			super().__init__(fmt)
			self.log_level = log_level

		def format(self, record):
			# Rebrand upstream browser-use log namespaces for WebPilot users.
			if isinstance(record.name, str) and record.name.startswith('browser_use.'):
				if 'Agent' in record.name:
					record.name = 'WebPilot'
					...
					if 'tools' in record.name:
						...
						record.name = 'WebPilot' if len(parts) >= 2 else parts[-1]
	...
	if log_type == 'result':
		...
		console.setFormatter(WebPilotLogFormatter('%(message)s', log_level))
	else:
		console.setLevel(log_level)  # Keep console at original log level (e.g., INFO)
		console.setFormatter(WebPilotLogFormatter('%(levelname)-8s [%(name)s] %(message)s', log_level))
	...
	if debug_log_file:
		...
		debug_handler.setFormatter(WebPilotLogFormatter('%(asctime)s - %(levelname)-8s [%(name)s] %(message)s', logging.DEBUG))
	...
	if info_log_file:
		...
		info_handler.setFormatter(WebPilotLogFormatter('%(asctime)s - %(levelname)-8s [%(name)s] %(message)s', logging.INFO))
