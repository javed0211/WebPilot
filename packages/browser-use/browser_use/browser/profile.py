...
def get_display_size() -> ViewportSize | None:
	...
	if sys.platform == 'darwin':
		# AppKit can terminate the entire process (SIGABRT) when Python runs without
		# a WindowServer session. Probe in a child process so headless runners can
		# safely fall back to the default viewport.
		try:
			probe = subprocess.run(
				[
					...
					'-c',
					(
						'import json; from AppKit import NSScreen; '
						'screen = NSScreen.mainScreen(); '
						'frame = screen.frame() if screen is not None else None; '
						'print(json.dumps([int(frame.size.width), int(frame.size.height)]) if frame else "null")'
					),
				],
				capture_output=True,
				text=True,
			...
			if probe.returncode == 0:
				dimensions = json.loads(probe.stdout.strip() or 'null')
				if dimensions:
					size = ViewportSize(width=dimensions[0], height=dimensions[1])
