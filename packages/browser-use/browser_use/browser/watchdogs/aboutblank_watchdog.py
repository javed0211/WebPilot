		...
		try:
			...
			script = f"""
				(function(browser_session_label) {{
					...
					const animated_title = `Starting WebPilot agent ${{browser_session_label}}...`;
					...
					loadingOverlay.style.background = '#0b1220';
					loadingOverlay.style.zIndex = '99999';
					loadingOverlay.style.overflow = 'hidden';

					// WebPilot wordmark (inline SVG data URI so it renders offline / without external hosts)
					const webpilotLogoSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="320" height="84" viewBox="0 0 320 84"><rect x="6" y="18" width="48" height="48" rx="12" fill="#2563eb"/><path d="M18 32 L26 54 L30 40 L34 54 L42 32" fill="none" stroke="#ffffff" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><text x="68" y="52" font-family="system-ui,-apple-system,Segoe UI,sans-serif" font-size="34" font-weight="700" fill="#f8fafc">Web<tspan fill="#60a5fa">Pilot</tspan></text></svg>';

					// Create the image element
					const img = document.createElement('img');
					img.src = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(webpilotLogoSvg);
					img.alt = 'WebPilot';
