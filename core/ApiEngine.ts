...
export class ApiEngine {
  ...
  public async execute(): Promise<EngineRunResult> {
    ...
    try {
      ...
      const reportsDir = ConfigManager.getInstance().get('framework.reportsPath', './reports');
      fs.mkdirSync(reportsDir, { recursive: true });
      const reportName = path.basename(this.testFilePath, path.extname(this.testFilePath));
      fs.writeFileSync(
        path.join(reportsDir, `api-${reportName}-${Date.now()}.json`),
      ...
      if (this.enableCodegen && result.success) {
        ...
        fs.mkdirSync(path.join(process.cwd(), 'framework', 'apis'), { recursive: true });
