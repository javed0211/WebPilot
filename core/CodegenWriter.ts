...
export class CodegenWriter {
  ...
  public static ensureSiteScaffolds(files: GeneratedFile[]): void {
    ...
    if (!fs.existsSync(basePath)) {
      ...
      console.log(
        `\x1b[32m[Codegen] Scaffolded site base Page Object: framework/pages/automationexercise/AutomationExerciseBasePage.ts\x1b[0m`
      );
    }
  }

  private static isPageObjectPath(filePath: string): boolean {
    return filePath.startsWith('framework/pages/') && filePath.endsWith('.ts');
  }
    ...
    if (MONOLITHIC_PAGE_DENYLIST.includes(base)) {
      console.error(
        `\x1b[31m[CodegenWriter] Rejected monolithic page "${file.path}". ` +
          `Split into per-route pages under framework/pages/<site>/ (see prompts/shared/framework-guidelines.md).\x1b[0m`
      );
      return true;
    }
    return false;
  ...
  private static resolvePageFilePath(file: GeneratedFile): string {
    ...
    if (file.path.includes('automationexercise') || className.startsWith('AutomationExercise')) {
      return `framework/pages/automationexercise/${className}.ts`;
    }
    return `framework/pages/${className}.ts`;
  }
