...
export function hasBrowserUse(pythonPath: string): boolean {
  try {
    execSync(`"${pythonPath}" -c "import browser_use"`, {
      stdio: 'pipe',
      env: { ...process.env, PYTHONPATH: path.join(projectRoot(), 'core') }
    });
    return true;
  } catch {
    return false;
...
/** Create .venv (if needed) and pip install requirements.txt. Returns venv python path. */
export function setupPythonVenv(systemPython?: string): string {
  ...
  const reqPath = path.join(root, REQUIREMENTS);
  if (!fs.existsSync(reqPath)) {
    throw new Error(`Missing ${REQUIREMENTS} at project root.`);
  }

  execSync(`"${venvPy}" -m pip install -U pip`, { stdio: 'inherit', cwd: root });
  execSync(`"${venvPy}" -m pip install -r "${reqPath}"`, { stdio: 'inherit', cwd: root });
  return venvPy;
}
...
export function ensureBrowserUsePython(): string {
  ...
  if (!hasBrowserUse(pythonPath)) {
    throw new Error(
      'Failed to install browser-use. Run manually:\n' +
